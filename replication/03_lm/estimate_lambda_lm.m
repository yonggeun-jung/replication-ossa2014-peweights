...
if params.use_multistart
    [lambda_opt, ~, ~, ~] = solve_multistart(j, TARIFFs, TARGET_ALL, MFN_guess, params, LAMBDA_OSSA);
else
    [lambda_opt, ~, ~, ~, ~] = solve_gn(j, TARIFFs, TARGET_ALL, MFN_guess, params);
end
...
T0 = compute_optimal_tariff(lam, j, TARIFFs, UB, MFN_guess, N, S);
r0 = compute_residuals(T0, tau, free_idx, prohib_idx, T_target, mean_target_free, UB);
obj = r0' * r0;
...
for it = 1:max_iter
    ...
    if isempty(J) || mod(it, broyden_refresh) == 0
        J = compute_jacobian_parallel(lam, j, TARIFFs, UB, MFN_guess, N, S, eps_fd);
    else
        s = lam - lam_prev;
        y = T0 - T_prev;
        denom = s' * s;
    ...
    for ls = 1:30
        ...
        T_try = compute_optimal_tariff(lam_try, j, TARIFFs, UB, MFN_guess, N, S);
        r_try = compute_residuals(T_try, tau, free_idx, prohib_idx, T_target, mean_target_free, UB);
        obj_try = r_try' * r_try;
...
function J = compute_jacobian_parallel(lam, j, TARIFFs, UB, MFN_guess, N, S, eps_fd)
...
parfor k = 1:S
    ...
    Tp = compute_optimal_tariff(lam_p, j, TARIFFs, UB, MFN_guess, N, S);
    Tm = compute_optimal_tariff(lam_m, j, TARIFFs, UB, MFN_guess, N, S);
    J(:, k) = (Tp - Tm) / (2 * eps_fd);
end
...
function T = compute_optimal_tariff(lam_, j, TARIFFs, UB, MFN_guess, N, S)
...
function r = compute_residuals(T, tau_val, free_idx, prohib_idx, T_target, mean_target_free, UB)
