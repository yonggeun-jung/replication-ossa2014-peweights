function [lambda_opt, tariff_opt, HISTORY] = estimate_lambda_gn_hist(j, TARIFFs, TARGET_ALL, LAMBDA_OSSA)
...
% Multistart optimization with history tracking
if params.use_multistart
    [lambda_opt, obj, grad_norm, iter, HISTORY] = solve_multistart(j, TARIFFs, TARGET_ALL, MFN_guess, params, LAMBDA_OSSA);
else
    [lambda_opt, obj, grad_norm, iter, ~, HISTORY] = solve_gn(j, TARIFFs, TARGET_ALL, MFN_guess, params);
end
...
function [lambda_best, obj_best, grad_best, iter_best, HISTORY_best] = solve_multistart(j, TARIFFs, TARGET_ALL, MFN_guess, params, LAMBDA_OSSA)
...
for k = 1:num_starts
    ...
    % Run optimization
    [lam_try, obj_try, grad_try, iter_try, ~, HIST_try] = solve_gn(j, TARIFFs, TARGET_ALL, MFN_guess, params, lam_init);
    
    % Keep best (lowest gradient norm)
    if grad_try < grad_best
        lambda_best = lam_try;
        obj_best = obj_try;
        grad_best = grad_try;
        iter_best = iter_try;
        HISTORY_best = HIST_try;
    end
end

end

% Core Gauss-Newton solver
function [lam, obj, grad_norm, it, exit_ok, HISTORY] = solve_gn(j, TARIFFs, TARGET_ALL, MFN_guess, params, lam_init)
